...
# ==== Variables globales para estabilización y turnos ====
REQUIRED_CONSECUTIVE_FRAMES = 3  # Número de frames consistentes requeridos
consecutive_count = 0
candidate_board = None
move_buffer = []  # Para almacenar los movimientos por turno
expected_turn = "W"  # Se asume que las blancas inician
...
def path_clear(board: List[List[str]], start: Tuple[int, int], end: Tuple[int, int]) -> bool:
    """Verifica que el camino entre start y end esté libre de obstáculos."""
    r1, c1 = start
    r2, c2 = end
    dr = r2 - r1
    dc = c2 - c1
...
def validate_move(old_board: List[List[str]], final_board: List[List[str]]) -> bool:
    """
    Valida que el movimiento detectado entre old_board y final_board cumpla:
      - La amazona se mueve en línea recta y su camino original esté libre.
      - Desde la nueva posición, el disparo de la flecha se efectúa en línea recta con camino libre.
    """
    moved_from = None
    moved_to = None
    arrow = None
    ...
    simulated_board[moved_to[0]][moved_to[1]] = moved_from[2]
...
# ----- Función detect_move (genera la notación web) -----
...
class BoardTracker:
    ...
    def get_amazons_positions(self, color: str) -> List[Tuple[int, int]]:
        return [pos for pos, piece in self.pieces.items() if piece.type == 'amazona' and piece.color == color]
